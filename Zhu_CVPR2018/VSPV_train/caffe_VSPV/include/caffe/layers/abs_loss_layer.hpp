#ifndef CAFFE_ABS_LOSS_LAYER_HPP_
#define CAFFE_ABS_LOSS_LAYER_HPP_
...
class AbsLossLayer : public LossLayer<Dtype> {
 public:
  explicit AbsLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "AbsLoss"; }
  /**
...
//      const vector<Blob<Dtype>*>& top);
  ...
  /**
   * @brief Computes the Abs error gradient w.r.t. the inputs.
   *
...
//      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
  ...
  Blob<Dtype> diff_;
  /// How to ingore label
  LossParameter_IgnoreMode ignore_mode_;
};

}  // namespace caffe

#endif  // CAFFE_ABS_LOSS_LAYER_HPP_
