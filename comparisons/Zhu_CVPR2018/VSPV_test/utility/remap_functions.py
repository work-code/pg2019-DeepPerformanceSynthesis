...
def forward2backwardMap(forward_flow):
    ...
    return backward_map
#xy=np.zeros((2,200,200))    
img_xy = np.zeros((2, 200, 200))
for iter_h in range(200):
    for iter_w in range(200):
...
i=1900
j=9    
#f=mpimg.imread('./final_image/1900/1900_9_3.png')
srcImg=mpimg.imread('/home/lzy/data/PVHM_0-9999/rendered_images/1900_9_200x200.png')[:,:,0:3]
srcImg = np.rollaxis(srcImg,2,0)
gt_flow_B = np.zeros((2,200,200))       
flow_u = np.fromfile(open('/home/lzy/data/PVHM_0-9999/backward_flow/{}_{}_u.bin'.format(i,j)), dtype = np.double)
flow_u.resize(200,200)
gt_flow_B[0,:,:] = flow_u.transpose()
flow_v = np.fromfile(open('/home/lzy/data/PVHM_0-9999/backward_flow/{}_{}_v.bin'.format(i,j)), dtype = np.double)
flow_v.resize(200,200)
gt_flow_B[1,:,:] = flow_v.transpose()
print np.max(np.abs(gt_flow_B))
gt_flow_B=gt_flow_B+img_xy
tgtImg=remap(gt_flow_B, srcImg)
print(np.max(np.abs(tgtImg-srcImg)))