...
def valSpatial(model,val_loader,val_dir,observations):
    ...
    with torch.no_grad():
        for batch_idx, (x, v) in enumerate(val_loader):
            ...
            if tot%20 == 0:
                scene = 'scene' + str(scene_tot + 1)
                print(scene)
                scene_dir = val_dir + '/Scene' + str(scene_tot+1)
                if scene_tot == 200:
                    return
                if os.path.exists(scene_dir) == False:
                    os.mkdir(scene_dir)
                scene_tot = scene_tot + 1
            index = tot%20 + 1
            save_image(x_mu[0,:,:,:].float(), scene_dir + '/query' + str(index) + '.jpg')
            save_image(x_q[0,:,:,:].float(), scene_dir + '/groundtruth' + str(index) + '.jpg')
            x_o = x_o[0,:,:,:,:]
            for l in range(0,len(x_o)):
                save_image(x_o[l,:,:,:].float(), scene_dir + '/observation_'+str((representation_idx[l].numpy()))+'.jpg')
            tot = tot + 1
...
def main(step,dataset,data_dir):
    ...
    val_loader = torch.utils.data.DataLoader(ShepardMetzler(root_dir=data_dir + '/torch' + '/val/spatial/'),#, target_transform=transform_viewpoint
